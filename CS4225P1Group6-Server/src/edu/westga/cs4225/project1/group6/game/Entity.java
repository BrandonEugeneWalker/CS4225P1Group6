...
/**
 * This abstraction represents any entity in the game. This could be a player or
 * an enemy. Different concrete implementations can partially override or fully
 * override the behavior that entities have. For example, bosses could override
 * the take damage method to always take reduced damage. Other modifications to
 * the concrete implementations are up to ones imagination.
 * 
...
public abstract class Entity {
	...
	/**
	 * Entities all start with a base health value and a base mana value. They also
	 * all have a particular role. The role given to an entity to determines the
	 * moveset that it has as well as what multipliers are given to health and mana
	 * values.
	 * 
	 * @precondition role != null
	 * @postcondition getHealthRemaining() == Entity.BASE_HEALTH *
	 *                role.getHealthMultiplier() && getManaRemaining() ==
	 *                Entity.BASE_MANA * role.getManaMultiplier() && isAlive() &&
	 *                !isDead() && if role.getManaMultiplier() > 0, then
	 *                !isOutOfMana() && if role.getManaMultiplier() == 0, then
	 *                isOutOfMana()
	 * 
	...
	/**
	 * Performs this entity's primary move. The primary move stems from this
	 * entity's role.
	 * 
	 * If the move succeeds, then the amount of damage done by the move is
	 * subtracted from the target's remaining health. Whether or not it succeeds
	 * depends on the moves accuracy value. A random number is generated between 0
	 * <= number < 1. If that number is less than or equal to the move's accuracy
	 * value, then the move succeeds; otherwise, it fails.
	 * 
	 * It does not matter if the move hits or not, the mana cost is subtracted from
	 * this entity's remaining mana. If this entity does not have enough mana to
	 * perform the move, then an exception is thrown.
	 * 
	 * @precondition target != null
	 * @postcondition getManaRemaining() <= getManaRemaining()@prev
	 * 
	 * @throws NotEnoughManaException if this entity does not have enough mana to
	 *                                perform the move.
	 * 
	...
	/**
	 * Performs this entity's special move. The special move stems from this
	 * entity's role.
	 * 
	 * For more information about how this method works, read the documentation for
	 * the performPrimaryMove(Entity).
	 * 
	 * @precondition target != null
	 * @postcondition getManaRemaining() <= getManaRemaining()@prev
	 * 
	...
	/**
	 * Makes this entity take the specified amount of damage. A negative damage
	 * value will increase the health remaining of this entity.
	 * 
	 * @precondition none
